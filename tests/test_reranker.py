...
async def test_rerank_sources_success(mock_client: MagicMock):
    ...
    assert result["best_course"]["title"] == "SQL for Beginners - Coursera"
    assert (
        result["best_course"]["url"]
        == "https://www.coursera.org/learn/sql-for-beginners"
    )
    assert result["best_course"]["raw_content"] is not None

    assert result["best_video"]["title"] == "Database Fundamentals - YouTube"
    assert result["best_video"]["raw_content"] is not None

    assert result["best_blog"]["title"] == "The Beginner Guide to Databases"
...
async def test_rerank_sources_invalid_llm_response(mock_client: MagicMock):
    ...
    with pytest.raises(ValueError, match="LLM returned an invalid response format"):
